        ...
        if req.status_code < 200 or req.status_code > 299:
            ...
            if r.status_code >= 200 and r.status_code <= 299:
                logger.info("Created Repo: "+repo_name +
                            ", with phisical location: "+location)
            else:
...
def CreateSnapshotFull(es_url, repo, auth):
    ...
    try:
        r = requests.put(
            ...
            utils.GetCurrDateTime()+'?wait_for_completion=true',
            auth=auth,
            headers={'Content-Type': 'application/json'})
        if r.status_code >= 200 and r.status_code <= 299:
    ...
    try:
        ...
        for snap in req_all_snap.json()['snapshots']:
            if index != "" and str(index) in str(snap['snapshot']) and str(snap['snapshot']) != snapshot:
                DeleteSnapshot(es_url=es_url, repo=repo,
...
def RestoreFromSnapshot(es_url, repo, index, auth):
    ...
    """
    try:
        snapshot_name = FindNewestSnapshot(
