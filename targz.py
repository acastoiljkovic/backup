...
def Targz(paths,destination,encrypt,encPass):
    for dir2compress in paths:
        ...
        fileName = filePrefix + '-' + utils.GetCurrTime() + '.tar.gz'
        fn = fileName
        ...
        if len(destination) > 1:
            for i in range(0, len(destination)):
                ...
                file_management.Rmold(dir, filePrefix)


def TargzIncremental(paths,destination,encrypt,encPass):
    for dir2compress in paths:
        logger.info("---------------------------------------")
        ...
        if len(destination) > 1:
            for i in range(1, len(destination)):
                file_management.Copy(fileName, destination[i], fn)
                file_management.Copy(snapFile, destination[i], filePrefix + '.snap')



def TargzDifferential(paths,destination,encrypt,encPass):
    for dir2compress in paths:
        logger.info("---------------------------------------")
        ...
        if len(destination) > 1:
            for i in range(0, len(destination)):
                if i != 0:
                    file_management.Copy(fileName, destination[i], fn)
                    file_management.Copy(snapFile, destination[i], filePrefix + '.snap')
                    file_management.Copy(snapFileBak, destination[i], filePrefix + '.snap.bak')
                file_management.KeepOnlyOldestAndNewest(dir=destination[i],name=filePrefix, encrypt=encrypt)